...
def LeagueTable(request):
    ...
    if request.method == 'GET':
        ...
        if _leagueTable == None:
            return Response("NO EPL League Table Found", status=status.HTTP_204_NO_CONTENT)
    ...
    # UPDATE
        ...
        if(not matchResuls.is_valid() ):
            ...
            return Response({'success': False, 'data': jsonStr}, status=status.HTTP_406_NOT_ACCEPTABLE)
...
def UserLogin(request):
    if request.method == "POST":
        userLoginInfo = models.LoginInfo(request.data)
        ...
        # Valid Input
            ...
            return Response({'success': response != [], 'userInfo' : response}, status = status.HTTP_200_OK if response != [] else status.HTTP_203_NON_AUTHORITATIVE_INFORMATION)
