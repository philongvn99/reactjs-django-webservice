...
const LeagueResultsForm = () => {
    ...
    const handleSubmit = (event) => {
        ...
        if (away.length !== numOfMatch) alert('[ERROR]: Number of Home teams is not equal to Away ones');
        else {
            let teamIdList = []
            //let teamPointList = []
            let teamGSList = []
            ...
            Array.from({ length: numOfMatch }, (v, k) => k).map(k => {
                ...
                // if (score[`home${k}`] > score[`away${k}`]) teamPointList.push(3, 0)
                // else if (score[`away${k}`] > score[`home${k}`]) teamPointList.push(0, 3)
                // else teamPointList.push(1, 1);
                
                return null;
            })
            
            axios.put('http://localhost:8000/UnitedHome/league/table/', {
                id: teamIdList,
                //point: teamPointLis,
    ...
    return (
      <Container><Row style={{backgroundColor: '#000020'}}>
                ...
                <div style={style.boardStyle}>
                    ...
                    {   
                        <form onSubmit={handleSubmit}>
                            {renderScoreBoard(home.length > away.length ? away.length : home.length)}
                            <div style={style.scoreboardStyle}><input type="submit" value="Submit" style={style.submitButton}></input></div>
                        </form> 
                    }
                </div>
            </Col>
