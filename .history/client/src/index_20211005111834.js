...
axios.post(
    'http://localhost:8000/UnitedHome/login/', 
    {
        ...
        password: "ure8582"//passwordLogin
    }
)
.then( (res) => {
    console.log(res.data)
    if(res.data.success) {
        axios.get('http://localhost:8000/UnitedHome/login/holtby331')
        .then( (dat) => {
            ReactDOM.render(<UserProfile userInfo={dat.data.userInfo}/>, document.getElementById('login-dropdown'));
        })
        .catch( (err) => alert(err))
    }
